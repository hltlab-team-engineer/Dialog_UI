...
public class EncodedAudioRecorder extends AbstractAudioRecorder {
    ...
    public EncodedAudioRecorder(int audioSource, int sampleRate) {
        ...
        try {
            //audioSource: same with RawAudioRecorder
            int bufferSize = getBufferSize();//the same with RawAudioRecorder
            createRecorder(audioSource, sampleRate, bufferSize);
            int framePeriod = bufferSize / (2 * RESOLUTION_IN_BYTES * CHANNELS);//the same with RawAudioRecorder
    ...
    public synchronized byte[] consumeRecordingEncAndTruncate() {
        ...
        Log.i("Buffer len is",bytes.length+"");
        return bytes;
    }
