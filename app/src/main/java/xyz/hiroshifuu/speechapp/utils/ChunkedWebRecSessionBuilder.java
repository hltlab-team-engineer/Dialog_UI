...
import xyz.hiroshifuu.speechapp.commons.Caller;
import xyz.hiroshifuu.speechapp.commons.PackageNameRegistry;
...
public class ChunkedWebRecSessionBuilder {
    ...
    public ChunkedWebRecSessionBuilder(Context context, Bundle extras, ComponentName callingActivity) throws MalformedURLException {
        mContext = context;
//extras:{android.speech.extra.PARTIAL_RESULTS=true, android.speech.extra.LANGUAGE_MODEL=free_form, calling_package=xyz.hiroshifuu.speechapp}
        if (extras == null) {
            extras = new Bundle();
        }
        ...
        PendingIntent pendingIntent = IntentUtils.getPendingIntent(extras);//null

        if (callingActivity == null) {
            Caller caller1 = new Caller(pendingIntent, extras);
            mCaller = caller1.getActualCaller();
            //mCaller:xyz.hiroshifuu.speechapp
