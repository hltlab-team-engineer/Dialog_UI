...
public abstract class AbstractAudioRecorder implements AudioRecorder {
    ...
    protected AbstractAudioRecorder(int audioSource, int sampleRate, int recordingBufferLengthMillis, boolean alwaysListen) {
        ...
        mSamplesInOneSec = RESOLUTION_IN_BYTES * CHANNELS * mSampleRate;//16000

        mSamplesInOneMilliSec = (int) ((double) mSamplesInOneSec / 1000.0);//16
        mRecordingBufferLengthMillis = recordingBufferLengthMillis;//35000
    ...
    public synchronized byte[] consumeRecordingAndTruncate() {
        ...
        setRecordedLength(0);
        setConsumedLength(0);
        Log.i("Abstract","Reset the recording");
        return bytes;
    }
    ...
    protected int read(SpeechRecord recorder, byte[] buffer) {
        int len = buffer.length;//raw: 2560; Encode:9216
        int numOfBytes = recorder.read(buffer, 0, len);//raw: 1280 ; encord:9216;raw方式无法读满
        ...
        int status = getStatus(numOfBytes, len);//0
     ...
     */
    public void start() {
        Log.i("AbstractAudio","Start Recording");
        if (getSpeechRecordState() == SpeechRecord.STATE_INITIALIZED) {
            mRecorder.startRecording();
            //mRecorder.getRecordingState()为3
            if (mRecorder.getRecordingState() == SpeechRecord.RECORDSTATE_RECORDING) {
                setState(State.RECORDING);
                new Thread() {
                    public void run() {
                        recorderLoop(mRecorder);//用encodedAudioRecorder的
                    }
                }.start();//用于启动线程
            } else {
